...
import {paperlessCorrespondentResults, paperlessCorrespondentsResponse} from '../paperlessResponse.model';
...
export const fetchCorrespondents = async (): Promise<paperlessCorrespondentsResponse> => {
    try {
        ...
        showToast(Toast.Style.Failure, `Could not fetch correspondents ${error}`);
